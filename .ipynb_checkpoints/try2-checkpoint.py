...
# C. Additional Features (Interaction Terms)
...
def cost_function(X, y, w):
    """
    Calculates the mean squared error cost for the predictions vs. actuals.
    """
...
def gradient_descent_optim(X, y, w_init, lr, num_iters): # teta bedast miare be soorate iterative
    """
    Gradient Descent for linear regression with param history tracking.
    """
    X_arr = np.array(X, dtype=np.float64)
    y_arr = np.array(y, dtype=np.float64).reshape(-1, 1)
...
def newton_1st_order(X, y, w_init, lr, num_iters):
    """
    Newton's Method (First-Order variant / steepest descent).
    """
    X_arr = np.array(X, dtype=np.float64)
    y_arr = np.array(y, dtype=np.float64).reshape(-1, 1)
...
def newton_2nd_order(X, y, w_init, num_iters):
    """
    Newton's Method (Second-Order) with Hessian-based updates.
    """
    X_arr = np.array(X, dtype=np.float64)
    y_arr = np.array(y, dtype=np.float64).reshape(-1, 1)
...
def assess_performance(y_true, y_hat):
    """
    Returns MSE and R2 given true and predicted values.
    """
